...
fn proper_initialization() {
    ...
    let msg = InstantiateMsg {
        ...
        token_code_id: 123,
        fee_address: Some(String::from("terra1rmwsanjl4tple6k3fjtqgmaepfefdwzvr6hyff")),
        owner: owner.address(),
...
fn update_config() {
    ...
    // update config
    let fee_address = Some(String::from("terra1rmwsanjl4tple6k3fjtqgmaepfefdwzvr6hyff"));
    let generator_address = Some(String::from("terra1rmwsanjl4tple6k3fjtqgmaepfefdwzvr6hyff"));
...
fn create_pair() {
    ...
    let asset_infos = [
        AssetInfo::Token {
            contract_addr: Addr::unchecked(token_instance0.data.address.clone()),
        },
        AssetInfo::Token {
            contract_addr: Addr::unchecked(token_instance1.data.address.clone()),
        },
    ];
    ...
    assert_eq!(res.events[10].attributes[1], attr("action", "create_pair"));
    assert_eq!(
        res.events[10].attributes[2],
        attr("pair", format!("{}-{}", token_instance0.data.address.clone(), token_instance1.data.address.clone()))
    );
    ...
    // check address for query response
    // these addresses are deterministic, it should return the same. These are instances created through SubMsg in contract factory
    assert_eq!("terra1aakfpghcanxtc45gpqlx8j3rq0zcpyf49qmhm9mdjrfx036h4z5sxdcxm3", res.contract_addr.to_string());
    assert_eq!("terra1cnuw3f076wgdyahssdkd0g3nr96ckq8cwa2mh029fn5mgf2fmcms6lfq8w", res.liquidity_token.to_string());
}