...
import {APIParams} from "@terraclassic-community/terra.js/dist/client/lcd/APIRequester";
...
export async function performTransaction(terra: LCDClient, wallet: Wallet, msg: Msg) {
    ...
    if (isTxError(result)) {
        throw new TransactionError(Number(result.code), result.codespace, result.raw_log)
    }
    return result
}
...
export async function instantiateContract(terra: LCDClient, wallet: Wallet, admin_address: string, codeId: number, msg: object, label: string) {
    ...
    return result.logs[0].events[0].attributes.filter(element => element.key == '_contract_address' ).map(x => x.value);
}
